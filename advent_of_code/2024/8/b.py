...
def inplace_mark_antinodes(
    ...
    for frequency_antennas_coords in frequency_char_to_antennas_coords.values():
        frequency_antennas_coords_list = list(frequency_antennas_coords)
        for ai, coords_a in enumerate(frequency_antennas_coords_list):
            np_coords_a = np.array(coords_a)
            for coords_b in frequency_antennas_coords_list[ai + 1 :]:
                np_coords_b = np.array(coords_b)

                np_delta = np_coords_a - np_coords_b

                np_antinode_coords = np_coords_a.copy()
                while are_coords_in_map(
                    puzzle_map=puzzle_map,
                    coords=np_antinode_coords,
                ):
                    puzzle_map[*np_antinode_coords] = "#"
                    np_antinode_coords += np_delta

                np_antinode_coords = np_coords_b.copy()
                while are_coords_in_map(
                    puzzle_map=puzzle_map,
                    coords=np_antinode_coords,
                ):
